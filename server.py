...
def ssh_exec(cmd):
    try:
        ...
        global ssh_client
        ssh_client = None # Force reconnect on next call
        return "", str(e), 1

# ── Gamelist Parser (Maximaler Bild-Fokus) ───────────────────
        ...
        try:
            with sftp.open(path, 'r') as f:
                ...
                for game in root.findall('game'):
                    ...
                    if r_path:
                        # PRIORITÄT: 1. boxart -> 2. thumbnail -> 3. image
...
def api_roms():
    ...
    try:
        ...
        cmd = f'find {search} -maxdepth 3 -type f \\( -name "*.iso" -o -name "*.bin" -o -name "*.smc" -o -name "*.gba" -o -name "*.nds" -o -name "*.z64" -o -name "*.n64" -o -name "*.nes" -o -name "*.sfc" -o -name "*.wbfs" -o -name "*.3ds" -o -name "*.rvz" -o -name "*.ps3" -o -name "*.zip" -o -name "*.7z" -o -name "*.chd" -o -name "*.m3u" -o -name "*.cue" -o -name "*.wua" -o -name "*.rpx" -o -name "*.psx" \\) 2>/dev/null | head -n 5000'
        ...
        meta_cache = {}
        if system != 'all':
            meta_cache[system] = get_gamelist_map(system)
        ...
        for r in roms_raw:
            ...
            if sys_name not in meta_cache:
                meta_cache[sys_name] = get_gamelist_map(sys_name)
...
def api_media():
    ...
    try:
        sftp = get_ssh().open_sftp()
        # Versuche mehrere Quellen für Cover
        fallbacks = [path, path.replace('-thumb.png', '-image.png'), path.replace('/images/', '/media/images/'), path.replace('.png', '.jpg'), path.replace('.jpg', '.png')]
        data = None
        found_path = None
        for p in fallbacks:
            try:
...
def api_stats_stream():
    def generate():
        last_cpu = [0, 0] # [total, idle]
        while True:
            try:
                # CPU load via /proc/stat (Delta calculation)
...
def control():
    ...
    cmds = {'stop-game': 'pkill -9 retroarch; pkill -9 dolphin; pkill -9 pcsx2; pkill -9 yuzu; pkill -9 rpcs3', 'volume-up': 'batocera-audio setSystemVolume +5', 'volume-down': 'batocera-audio setSystemVolume -5', 'reboot': 'reboot', 'shutdown': 'poweroff'}
...
def api_settings():
    if request.method == 'GET':
        ...
        global ssh_client
        ssh_client = None # Force reconnect with new settings
        return jsonify({'ok': True})
...
def api_logs():
    ...
    # Define possible paths for each log type
    paths = {
...
def files():
    ...
    for line in out.split('\n')[1:]:
        p = line.split(None)
        if len(p) >= 8: res.append({'name': ' '.join(p[8:]), 'isDir': p[0].startswith('d'), 'size': p[4]})
